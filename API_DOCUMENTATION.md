...
> **Backend hardening pass (this revision):** no endpoints were added, removed,
> or renamed — this document already matched the implemented API. Fixes in
> this pass were internal: two MySQL-only `INDEX (...)` clauses in
> `database/schema.sql` were converted to standalone Postgres `CREATE INDEX`
> statements (Supabase runs Postgres and rejected the old syntax), a missing
> `users.reset_token` column used by the password-reset flow was added (see
> `database/migrations/003_add_reset_token.sql` for existing databases), a
> crash in `GET /admin/payments/stats` (`.group()` is not a real supabase-js
> method) was replaced with in-memory aggregation, an unused/dead duplicate
> of `routes/admin/users.js` was removed, and `package.json` / `.env.example`
> were added since neither existed before.
