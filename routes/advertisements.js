...
router.post('/', authMiddleware, uploadSingle, async (req, res) => {
  try {
    ...
    logActivity(req.user.id, 'advertisement_created', `Your advertisement "${advertisement.title}" was created and is awaiting payment`, { advertisement_id: advertisement.id });
  } catch (error) {
  ...
  try {
    ...
    if (paymentData.status === 'success' && existingPayment) {
      ...
      if (ad) {
        ...
        logActivity(ad.user_id, 'advertisement_payment_success', `Payment confirmed for "${ad.title}" — awaiting admin approval`, { advertisement_id: ad.id });
      }
    }
...
// NOTE: the global express.json() middleware in server.js already parses
// this request and stashes the raw bytes onto req.rawBody (via its
// `verify` hook) — this route does NOT use express.raw() itself, since
// by the time a route-level body parser would run here, the global one
// has already consumed the request stream and req.body is already a
// parsed object, not a Buffer.
router.post('/webhook/paystack', async (req, res) => {
  try {
    ...
    if (!req.rawBody) {
      console.error('Webhook error: raw body unavailable for signature verification');
      return res.status(500).json({ error: 'Unable to verify webhook' });
    }

    // Paystack signs the exact raw request bytes, so the HMAC must be
    // computed over req.rawBody, not a re-serialized copy of req.body —
    // JSON.stringify(req.body) can reorder keys/whitespace and will
    // never match Paystack's signature.
