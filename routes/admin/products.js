...
// Approve a pending/rejected product — becomes publicly visible (Part 2/13/17).
router.patch('/:id/approve', async (req, res) => {
  try {
    ...
    if (product.user?.id) {
      ...
      logActivity(product.user.id, 'product_approved', `Your listing "${product.name}" was approved and is now live`, { product_id: id });
    }
...
// Reject a product with a reason — stays hidden from the public marketplace.
  ...
  try {
    ...
    const { data: product, error } = await supabase
      ...
      .update({
        ...
        rejection_reason: reason || 'Did not meet marketplace guidelines',
        reviewed_by: req.user.id,
        reviewed_at: new Date().toISOString()
      })
      .eq('id', id)
    ...
    if (product.user?.id) {
      const { logActivity } = require('../../utils/activity');
      logActivity(product.user.id, 'product_rejected', `Your listing "${product.name}" was rejected: ${product.rejection_reason}`, { product_id: id });
    }
