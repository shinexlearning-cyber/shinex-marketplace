...
// Grant or revoke admin privileges (Part 13 "ADMIN — ADMIN MANAGEMENT").
// Reachable only by someone who already passed authMiddleware + adminMiddleware
// above (router.use), so a normal user can never reach this route no matter
// what their own request body claims — the backend, not the frontend, is
// the source of truth for is_admin.
router.patch('/:id/set-admin', async (req, res) => {
  try {
    const { id } = req.params;
    const { is_admin } = req.body;

    if (typeof is_admin !== 'boolean') {
      return res.status(400).json({ success: false, message: 'is_admin must be true or false' });
    }
    ...
    res.json({
      success: true,
      message: is_admin ? 'User granted admin access' : 'Admin access revoked',
      data: user
    });
  } catch (error) {
