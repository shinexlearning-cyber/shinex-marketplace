...
const { getPagination, buildPaginationResponse, isValidUUID, normalizeWhatsApp } = require('../utils/helpers');
...
router.put('/me', authMiddleware, async (req, res) => {
  try {
    const { full_name, bio, location, whatsapp, shop_name, shop_description, is_seller } = req.body;
    ...
    if (whatsapp !== undefined) {
      // Part 9: normalize + validate WhatsApp number server-side.
      const { valid, normalized, error } = normalizeWhatsApp(whatsapp);
      if (!valid) {
    ...
    if (is_seller !== undefined) updates.is_seller = !!is_seller;
