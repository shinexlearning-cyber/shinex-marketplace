...
router.post('/product/:productId', authMiddleware, async (req, res) => {
  try {
    ...
    logActivity(req.user.id, 'product_favorited', `You favorited "${product.name}"`, { product_id: productId });
  } catch (error) {
    console.error('Favorite product error:', error);
    res.status(500).json({
      success: false,
...
router.post('/seller/:sellerId', authMiddleware, async (req, res) => {
  try {
    ...
    logActivity(req.user.id, 'shop_favorited', 'You followed a shop', { seller_id: sellerId });
    logActivity(sellerId, 'new_follower', 'You received a new follower', { follower_id: req.user.id });
  } catch (error) {
    console.error('Favorite seller error:', error);
    res.status(500).json({
      success: false,
